...
export function RevenueSection({ stats, contracts }: RevenueSectionProps) {
  ...
  const revenueByCompany = contracts.reduce((acc: Record<string, number>, contract) => {
    ...
    <div className="space-y-6">
      ...
      <Card className="p-6 bg-gradient-to-br from-blue-50 to-blue-50/30">
        <div className="space-y-4">
          <div>
            <p className="text-sm font-medium text-muted-foreground">Receita Total (Contratos Ativos)</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {companyData.length > 0 && (
          <Card className="p-6">
            <h3 className="font-semibold mb-4">Receita por Empresa</h3>
      ...
      {companyData.length > 0 && (
        <Card className="p-6">
          <h3 className="font-semibold mb-4">Resumo de Receitas</h3>
              ...
              <div key={index} className="flex items-center justify-between pb-3 border-b last:border-b-0">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {contracts.filter(c => c.empresa === company.name).length} contrato(s)
                  </p>
                </div>
